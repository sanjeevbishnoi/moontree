...
class UnsignedTransactionService {
  ...
  UnsignedTransaction _create({
    ...
    ravencoin.TransactionBuilder txb = ravencoin.TransactionBuilder(
      network: ravencoin.mainnet, // how do we determin this? ravencoin.testnet
    );
    ...
    for (var unspent in unspents) {
      print(unspent.txId);
      txb.addInput(unspent.txId, unspent.position.getOrCrash());
    ...
    if (change.getOrCrash() > 0) {
      txb.addOutput(send.change, change.asSats);
    }
